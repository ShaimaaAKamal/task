...
export function handleRegister(){
        ...
        const handleChange=(e)=>{
          ...
          if(e.target.value === '' && !e.target.nextElementSibling.classList.contains('d-none'))  
            { e.target.nextElementSibling.classList.add('d-none');
                      formInputFormat(e,'#98AEF8') 
            }
          else if(!chooseValidate(e.target.name,e.target.value)) 
            {e.target.nextElementSibling.classList.remove('d-none');
                  formInputFormat(e,'#DC3545')  
            }   
          else 
              {e.target.nextElementSibling.classList.add('d-none');
              formInputFormat(e,'#009402') 
              }
            formValidate();
        }
        ...
        const formInputFormat=(e,color)=>{
              e.target.style.borderColor=color;
              e.target.nextElementSibling.nextElementSibling.style.color=color; 
        }
